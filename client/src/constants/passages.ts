...
export const getPassage = (
  ...
  if (isPractice) {
    ...
    for (const chars of practiceStrings) {
      const amountOfWords = Math.ceil(passageLength / practiceStrings.length);
      if (chars.length > 1) {
        passageWords = passageWords.concat(
          new Array(amountOfWords).fill(chars.replaceAll("_", ""))
        );
        continue;
      }
      ...
      if (filteredWords.length > 0) {
        for (let i = 0; i < amountOfWords; i++) {
          ...
          passageWords.push(newWord);
        }
      }
    }
    return shuffle(passageWords).join(" ");
  } else {
    let newPassage = "";
